...
int main(int argc, char *argv[]) {
	...
	temp = SDL_LoadBMP("sd:/apps/pong/numbermap.bmp");
	...
	//load the numbermap image strip of 10 number 64px * 64px
	temp = SDL_LoadBMP("sd:/apps/pong/title.bmp");
	...
	//load the numbermap image strip of 10 number 64px * 64px
	temp = SDL_LoadBMP("sd:/apps/pong/gameover.bmp");
	...
	while (quit == 0) {
		...
		if ( WPAD_ButtonsDown(0) & WPAD_BUTTON_HOME )  quit = 1;
		if ( WPAD_ButtonsDown(0) & WPAD_BUTTON_A )     if (state == 0) state = 1;
		if ( WPAD_ButtonsDown(0) & WPAD_BUTTON_PLUS )  move_paddle(1); // up
		if ( WPAD_ButtonsDown(0) & WPAD_BUTTON_MINUS ) move_paddle(0); // down
		if ( WPAD_ButtonsDown(0) & WPAD_BUTTON_UP )    move_paddle(1); // up
		if ( WPAD_ButtonsDown(0) & WPAD_BUTTON_DOWN )  move_paddle(0); // down
    	...
    	* look for an event *
    	if (SDL_PollEvent(&event)) {
			...
			case SDL_MOUSEBUTTONDOWN:
            	//if (state == 0) state = 1;
            	//else paddle[1].y = event.motion.y;
				break;
			case SDL_MOUSEBUTTONUP:
				break;
			case SDL_MOUSEMOTION:
				//paddle[1].y = event.motion.y;
				break;
