...
SDL_Event event;
// Create a rect at pos ( 5, 5 ) that's 5 pixels wide and 5 pixels high. Show tilt activation
SDL_Rect rect;
bool running = true;
bool tilt = false; // tilt sensor - de-/activated with button B
...
void updateDisplay() {
    ...
    // Render rect to show tilt activation
    if (tilt) {
...
void handleKeyPress() {
		...
		if ( pressed & WPAD_BUTTON_B ) tilt = !tilt; // tilt sensor - de-/activated with button B
...
// Create a rect at pos ( 0, 5 ) that's 15 pixels wide and 15 pixels high. Show tilt activation
    rect.x = 0;
    rect.y = 5;
    rect.w = 15;
    rect.h = 15;
